...
// --- ICONS (Inline SVGs for instant professional look) ---
const Icons = {
...
export default function PlanPage() {
  ...
  // Display Name Logic
  const displayName = profile.name || session?.user?.name?.split(' ')[0] || "Yogi";
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] text-slate-800 font-sans relative overflow-hidden">
      ...
      {/* --- Ambient Background (OPTIMIZED CLASSES) --- */}
      <div className="fixed top-0 left-0 w-full h-full pointer-events-none">
        {/* w-[500px] -> w-125, h-[500px] -> h-125 */}
        <div className="absolute top-[-10%] right-[-5%] w-125 h-125 bg-teal-200/20 rounded-full blur-[100px]" />
        {/* w-[600px] -> w-150, h-[600px] -> h-150 */}
        <div className="absolute bottom-[-10%] left-[-10%] w-150 h-150 bg-orange-100/30 rounded-full blur-[120px]" />
      </div>
      ...
      <div className="max-w-6xl mx-auto p-6 md:p-12 relative z-10">
        ...
        <header className="flex justify-between items-start mb-12">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="text-4xl md:text-5xl font-light text-slate-900 tracking-tight">
              Namaste, <span className="font-semibold text-teal-800">{displayName}</span>
            </h1>
          </motion.div>
        ...
        {plan && (
          <motion.div 
            ...
            // OPTIMIZED: rounded-[2rem] -> rounded-4xl
        ...
        <div className="grid lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="relative space-y-4">
              {/* Timeline Line (OPTIMIZED: left-[28px] -> left-7, w-[2px] -> w-0.5) */}
              ...
              {plan?.routine?.map((pose: any, i: number) => (
                ...
                >
                  ...
                  <div className="flex-1 p-5 rounded-3xl bg-white/70 backdrop-blur-md border border-white/60 shadow-sm hover:shadow-md transition-all duration-300 hover:bg-white/90 cursor-default">
                    ...
                    {/* OPTIMIZED: Removed 'flex' (redundant with inline-flex) */}
                    <div className="inline-flex items-center gap-2 text-xs font-medium text-teal-600 bg-teal-50/50 px-3 py-1.5 rounded-lg">
                      <Icons.Fire /> 
                      {pose.benefit}
                    </div>
                  </div>
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            {/* Mindfulness Widget (OPTIMIZED: rounded-[2.5rem] is roughly rounded-4xl/5xl but 4xl is standard) */}
            <motion.div 
              initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}
              className="p-8 rounded-4xl bg-teal-900 text-white shadow-xl relative overflow-hidden"
            >
            ...
            <motion.div 
              ...
              className="p-8 rounded-4xl bg-white border border-slate-100 shadow-sm"
            >
            ...
            <motion.button
              ...
              // OPTIMIZED: bg-linear-to-r
              className="w-full py-5 rounded-2xl bg-linear-to-r from-teal-600 to-teal-500 text-white font-bold text-lg shadow-lg shadow-teal-200/50 hover:shadow-xl transition-all cursor-pointer"
            >
